      ...
      <div class="container">
        ...
        <div class="single">
          ...
          <div class="image-wrap">
            <img id="singleImage" src="${escapeHtml(CONFIG.MEME_BASE_URL + CONFIG.IMAGE_DIRECTORY + encodeURIComponent(item.File || ''))}" alt="${escapeHtml(item.Name || '')}">
  ...
  function renderGrid(items, q, mount) {
    ...
    mount.innerHTML = `<div class="container">${header}<h1>Results${q ? ' for "' + escapeHtml(q) + '"' : ''}</h1><div class="grid">${items.map(it => `
      <a class="card" href="?id=${encodeURIComponent(it.Id || it.File || '')}" title="${escapeHtml(it.Name || '')}">
        <img src="${escapeHtml(CONFIG.MEME_BASE_URL + CONFIG.IMAGE_DIRECTORY + encodeURIComponent(it.File || ''))}" alt="${escapeHtml(it.Name || '')}">
